...
namespace SparkEngine.Pathfinding
{
    public class PathNode
    {
        ...
        {
            var xManhattan = (int) Math.Abs(Coordinates.X - target.Coordinates.X);
            var yManhattan = (int) Math.Abs(Coordinates.Y - target.Coordinates.Y);
            Heuristic = TileCostAdjacent * (xManhattan + yManhattan) +
                        (TileCostDiagonal - 2 * TileCostAdjacent) * Math.Min(xManhattan, yManhattan);
            //Heuristic *= 1 + 0.02;
